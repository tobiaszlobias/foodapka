    ...
    <div className="min-h-screen bg-surface dark:bg-black transition-colors">
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-6 py-10 md:py-16">
        <header className="mb-10 text-left">
          <h1 className="text-3xl md:text-5xl font-display font-extrabold tracking-tight text-foodappka-950 dark:text-white leading-tight mb-3">
            Recepty <span className="text-foodappka-600 dark:text-foodappka-400">s chytrým nákupem</span>
          </h1>
        ...
        <section className="grid gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {RECIPE_PRESETS.map((recipe) => (
            <Link
              ...
              className="group rounded-2xl bg-white dark:bg-foodappka-950 border border-zinc-100 dark:border-zinc-800 overflow-hidden shadow-sm transition-all hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.98]"
            >
              {recipe.image && (
              ...
              <div className="p-5">
                <h2 className="text-lg font-black text-zinc-900 dark:text-white leading-tight group-hover:text-foodappka-600 transition-colors">
                  {recipe.name}
                </h2>
        ...
        <div className="mt-12 rounded-[2rem] bg-foodappka-950 p-8 md:p-10 text-white text-center">
          <h2 className="text-xl md:text-2xl font-black mb-2">Nenašli jste svůj recept?</h2>
          <p className="text-sm md:text-base text-foodappka-200 mb-6">
            V aplikaci si můžete vytvořit vlastní recept nebo si ho nechat vygenerovat od AI.
          </p>
          <Link
            href="/app?mode=recipes"
            className="inline-flex items-center gap-2 rounded-full bg-foodappka-500 px-8 py-3 font-black text-white transition hover:bg-foodappka-600 active:scale-95"
          >
            Otevřít aplikaci
          </Link>
        </div>
